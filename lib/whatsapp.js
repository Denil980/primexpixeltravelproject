...
export function buildWhatsAppBookingMessage(payload) {
  const parts = [
    ...
    `• *Phone / WhatsApp:* ${payload.customerPhone || 'Not provided'}`,
    payload.travelDate ? `• *Expected Travel Date:* ${payload.travelDate}` : '',
    payload.guests ? `• *Number of Guests:* ${payload.guests}` : '',
  ];

  if (payload.notes && payload.notes.trim()) {
    parts.push(`\n📝 *Special Requests:*`, payload.notes.trim());
  }
